...
//  Dictionary
//
//  Created by Pradeep Rajkumar on 08/05/18.
...
//    NSArray *A = @[@1,@3,@3,@4,@1];
    ...
    NSLog(@"%ld", (long)[self repeatedNumber:A]);
}
...
-(NSInteger) repeatedNumber:(NSArray *) A  {
    ...
    for (int index = 0;  index < A.count; index++) {
        if ([([dictionary objectForKey:[NSString stringWithFormat:@"%@",[A objectAtIndex:index]]]) isEqual:@1]) {
            NSInteger returnValue = [A objectAtIndex:index];
            return returnValue;
        } else {
            [dictionary setObject:@1 forKey:[NSString stringWithFormat:@"%@",[A objectAtIndex:index]]];
        }
    }
    return (NSInteger)-1;
}
...
-(NSInteger) repeatedNumber:(NSArray *) A  {
    NSMutableArray *a = A.mutableCopy;
    for(int i = 0; i < a.count; i++) {
        NSNumber * n = [a objectAtIndex:i];
        NSInteger ind = abs(n.intValue);
        NSNumber *atInd = [a objectAtIndex:ind];
        if(atInd.intValue < 0)
            return ind;
        [a replaceObjectAtIndex:ind withObject:@(-atInd.intValue)];
    }
}
...
-(NSInteger) repeatedNumber:(NSArray *) A  {
    ...
    B = [A mutableCopy];
    for (int index = 0;  index < A.count; index++) {
        NSInteger value = ((NSNumber *)[A objectAtIndex:index]).intValue;
        NSInteger newValue = ((NSNumber *)[B objectAtIndex:value]).intValue;
        if (newValue < 0) {
            return value;
        }
        else {
